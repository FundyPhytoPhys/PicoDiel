...
select(-c("data", "lrcplatt_Model","lrcplatt_Predict", "lrcplatt_Glance")) |>
...
MCDataHour2 <- MCDataHour |>
  ...
  left_join(Processed_Solisense_LRC, by = c("SampleID")) |> mutate(JVPSII_eLh = Pmax_445 * (1 - exp(-a_445*Actinic_umolphotonsm2h * 6.022e17/Pmax_445)) * exp(-b_445*Actinic_umolphotonsm2h  * 6.022e17/Pmax_445))
...
MCDataHour2 |>
  ...
  # geom_point(aes(x = Actinic_umolphotonsm2h, y = mu_Averh, colour = ToD)) +
  # scale_color_gradient(low = "black", high = "yellow") +
  geom_point(aes(x = Actinic_umolphotonsm2h, y = JVPSII_eLh, colour = ToD)) +
  facet_grid(cols = vars(Tube)) +
  theme_bw()
```
