...
MCDataHour <- MCData |>
  ...
  mutate(mu_OD720window = zoo::rollapply(cbind(time, log(OD720)), width = 60/ODinterval, FUN = slope, by.column=FALSE, fill=NA)) |> #slope
  mutate(mu_Averh = zoo::rollmean(mu_OD720window, k = 60/ODinterval, fill = NA, align = "right")) #average of first 12 row
...
MCDataHour |>
  ...
  #filter(time < 100) |>
  ggplot() +
  geom_point(aes(x = time, y = mu_OD720window, colour = Actinic_umolphotonsm2h)) +
...
MCDataHour |>
   ...
   filter(time <= 96) |>
  ggplot() +
  geom_point(aes(x = Actinic_umolphotonsm2h, y = mu_OD720window, colour = ToD)) +
...
MCDataHour |>
   ...
   filter(time <= 96) |>
  ggplot() +
  geom_point(aes(x = ToD, y = mu_OD720window, colour = Actinic_umolphotonsm2h)) +
  geom_line(aes(x = ToD, y = mu_OD720window, colour = Actinic_umolphotonsm2h)) +
  scale_color_gradient(low = "black", high = "yellow") +
  facet_grid(cols = vars(Tube)) +
  theme_bw()
...
MCDataHour |>
  ...
  geom_line(aes(x = ToD, y = mu_OD720window, colour = Actinic_umolphotonsm2h)) +
  scale_color_gradient(low = "black", high = "yellow") +
  geom_smooth(aes(x = ToD, y = mu_OD720window)) +
  facet_grid(cols = vars(Tube)) +
  theme_bw()
...
MCDataHourSVR <- MCDataHour %>%
  select(c(time, ToD, Day, mu_OD720window, Actinic_umolphotonsm2h,  SampleID, Strain, Tube, Run, Par_ue, Photoperiod, OD720, OD680, deltaOD, Actinic_par, WL, O2)) %>%
  drop_na(mu_OD720window) %>% 
  drop_na(Actinic_umolphotonsm2h)


# Function to train an SVM model and make predictions
train_and_predict_svm <- function(data) {
  svr_model <- svm(mu_OD720window ~ time, data = data, kernel = 'radial',
                   cost = 24, gamma = 100)
...
RMSE_by_Tube <- result %>%
  group_by(Tube, Run) %>%
  summarise(RMSE = sqrt(mean((mu_OD720window - svr_pred)^2)))
...
MCDataHourSVR %>%
  ...
  ggplot() +
  geom_point(aes(x = time, y = mu_OD720window), alpha = 0.4) +
  geom_line(aes(x = time, y = svr_pred, color = 'SVR_radial')) +
  geom_line(aes(x = time, y = mu_OD720window)) +
