...
MaxMuLightData <- DielData |>
  filter(Run %in% c(39, 40, 43, 44, 45, 46, 50, 60, 62, 65)) |> 
  filter(time >= 24) |>
  filter(time < 96) |>
  group_by(Filename, Tube, Day) |>
  mutate(MinMu = min(mu_Averh, na.rm = TRUE),
         MaxMu = max(mu_Averh, na.rm = TRUE),
         MinMuToD = ToD[which(mu_Averh == min(mu_Averh, na.rm = TRUE))],
         MaxMuToD = ToD[which(mu_Averh == max(mu_Averh, na.rm = TRUE))],
         LightMaxToD = ToD[which(Actinic_umolphotonsm2h == max(Actinic_umolphotonsm2h, na.rm = TRUE))],
...
MaxMuLightData |>
   ggplot() +
  geom_point(aes(x =ToD, y  = Actinic_umolphotonsm2h/100000), colour = "Yellow") +
...
MaxMuLightData |>
  ggplot() +
  geom_point(aes(x = Par_ue, y  = deltaMaxMuLightToD, colour = as.factor(Run))) +
  geom_smooth(aes(x = Par_ue, y  = deltaMaxMuLightToD), method = "lm") +
  #geom_point(aes(x = LightMaxToD, y  = MaxMuToD, colour = MaxMu)) +
  #coord_cartesian(xlim = c(0, 24)) +
